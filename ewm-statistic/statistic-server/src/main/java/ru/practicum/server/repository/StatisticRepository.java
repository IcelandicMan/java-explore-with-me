...
public interface StatisticRepository extends JpaRepository<HitEntity, Long> {
    ...
    @Query("SELECT new ru.practicum.dto.ResponseHitDto(h.app, h.uri, COUNT(h.ip)) " +
            "FROM HitEntity AS h " +
    ...
    @Query("SELECT new ru.practicum.dto.ResponseHitDto(h.app, h.uri, COUNT(DISTINCT h.ip)) " +
            "FROM HitEntity AS h " +
            "WHERE h.timestamp BETWEEN :start AND :end " +
            "GROUP BY h.app, h.uri " +
